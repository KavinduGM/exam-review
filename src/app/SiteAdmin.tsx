...
export function SiteAdmin({ sites }: { sites: SiteRow[] }) {
  ...
  const [form, setForm] = useState({ key: "", name: "", baseUrl: "", sitemapUrl: "", defaultSets: 5, defaultParts: 3, defaultTimedSets: 5 });
  ...
  async function add(e: React.FormEvent) {
    ...
    if (res.ok) {
      setOpen(false);
      setForm({ key: "", name: "", baseUrl: "", sitemapUrl: "", defaultSets: 5, defaultParts: 3, defaultTimedSets: 5 });
      router.refresh();
    } else {
  ...
  return (
    <>
      ...
      <table>
        <thead><tr><th>Key</th><th>Name</th><th>Base URL</th><th>Structure</th><th>Exams</th><th>Status</th><th></th></tr></thead>
        <tbody>
          {sites.map((s) => (
            <tr key={s.id}>
              <td>{s.key}</td>
              ...
              <td className="muted">{s.defaultSets}×{s.defaultParts} practice · {s.defaultTimedSets} timed</td>
              <td>{s.exams}</td>
              <td>
